...
class MegatronRewardModel(BasePPORewardModel):
    ...
    def re_encode_by_rm_tokenizer(self, data: DataProto) -> DataProto:
        ...
        # 2. decode by the input tokenizer and convert to the reward-model chat template
        ...
        for id, mask in zip(input_ids, attention_mask, strict=True):
            ...
            # 2. decode by the input tokenizer
            decode_result = self.input_tokenizer.decode(valid_id)
            # workaround
            decode_with_rm_chat = (
            ...
            if print_decode and torch.distributed.get_rank() == 0:
                ...
                print(
                    f"device {get_device_id()}: input decode result:\n{decode_result}\n \
                        \ndevice {get_device_id()}: input decode result with \
                        rm chat template:\n{decode_with_rm_chat}\n\n"
                )
                print_decode = False
            ...
            if cur_seqlen > ori_seqlen:
                print(f"warninig: rm encode seqlen {cur_seqlen} > input encode seqlen {ori_seqlen}")
