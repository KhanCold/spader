...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def _handle_generating_state(
        ...
        if agent_data.tool_calls:
            # Check if the parser returned a parse-error sentinel instead of
            # a real tool call.  When this happens we feed the error back to
            # the model as a tool response so it can retry, rather than
            # silently terminating the turn.
            if (
                len(agent_data.tool_calls) == 1
                and agent_data.tool_calls[0].name == "__parse_error__"
            ):
                parse_err = json.loads(agent_data.tool_calls[0].arguments)
                error_msg = (
                    f"Your tool call could not be parsed. "
                    f"Error: {parse_err['error']}. "
                    f"Please output valid JSON inside <tool_call>...</tool_call> tags and try again."
                )
                add_messages = [{"role": "tool", "content": error_msg}]
                agent_data.messages.extend(add_messages)
