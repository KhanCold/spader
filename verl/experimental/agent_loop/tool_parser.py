...
# Qwen tool-calling (as used in repo examples) currently emits the same
# <tool_call>{...}</tool_call> pattern as the Hermes parser expects.
@ToolParser.register("qwen")
class QwenToolParser(HermesToolParser):
    pass


@ToolParser.register("text")
class TextToolParser(ToolParser):
    """Parse plain-text tool calls: <tool_call>["query1", "query2"]</tool_call>.

    Unlike Hermes/Qwen parsers that expect structured ``{"name": ..., "arguments": ...}``
    dicts, this parser handles the "prompt-injected" style where the model outputs a
    bare JSON list inside ``<tool_call>`` tags.  Each parsed list is normalised into a
    ``FunctionCall(name="search", arguments={"query_list": [...]})`` so the downstream
    ``ToolAgentLoop`` can execute it without any changes.

    Also supports the legacy dict format as a fallback:
    ...
    def _normalize_query_list(self, value) -> list[str]:
        ...
        for item in value:
            if isinstance(item, str):
                ...
                normalized.extend(self._normalize_query_list(item))
        return normalized
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        # Only parse the LAST <tool_call> block, consistent with inference.py.
        # The model may echo earlier tool_call examples inside <think>, so
        # taking all matches would produce spurious calls.
        function_calls: list[FunctionCall] = []
        last_match = matches[-1].strip()
        try:
            parsed = json.loads(last_match)
            if isinstance(parsed, list):
                # Bare list format: ["q1", "q2"] → wrap as search tool call
                normalized_queries = self._normalize_query_list(parsed)
                function_calls.append(
                    FunctionCall(
                        name="search",
                        arguments=json.dumps({"query_list": normalized_queries}, ensure_ascii=False),
                    )
                )
            elif isinstance(parsed, dict):
                # Legacy dict format: {"name": "search", "arguments": {...}}
                name = parsed.get("name", "search")
                arguments = parsed.get("arguments", parsed)
                if isinstance(arguments, dict) and "query_list" in arguments:
                    arguments = dict(arguments)
                    arguments["query_list"] = self._normalize_query_list(arguments.get("query_list"))
                function_calls.append(
                    FunctionCall(name=name, arguments=json.dumps(arguments, ensure_ascii=False))
                )
            else:
                logger.warning(f"Unexpected tool_call payload type: {type(parsed)}")
        except Exception as e:
            logger.error(f"Failed to parse text tool call: {e}")
            # Return a sentinel FunctionCall so the agent loop can feed the
            # error back to the model instead of silently terminating.
            function_calls.append(
                FunctionCall(
                    name="__parse_error__",
                    arguments=json.dumps(
                        {"raw_text": last_match, "error": str(e)},
                        ensure_ascii=False,
                    ),
                )
            )
