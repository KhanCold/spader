...
class SearchExecutionWorker:
    ...
    def _init_rate_limit(self, rate_limit):
        ...
        # NOTE:
        # - If this actor is NOT detached, Ray will treat it as "owned" by the creating worker.
        #   When that worker crashes/restarts, the actor will be killed and tool calls can hang.
        # - We also allow infinite restarts/task retries to improve robustness for long training jobs.
    ...
    def _reinit_rate_limit(self):
        if not self.enable_global_rate_limit:
            self.rate_limit_worker = None
            return
        try:
            if self.rate_limit_worker is not None:
                ray.kill(self.rate_limit_worker, no_restart=True)
        except Exception:
            pass
        self.rate_limit_worker = self._init_rate_limit(self.rate_limit)
    ...
    def execute(self, fn: Callable[..., T], *fn_args, **fn_kwargs) -> T:
        ...
        try:
            try:
                ...
                logger.warning(
                    "[SearchTool] rate limiter actor failed (%s); recreating and retrying acquire.",
                    e,
                )
                self._reinit_rate_limit()
                ray.get(self.rate_limit_worker.acquire.remote())
                acquired = True
            ...
            if acquired and self.rate_limit_worker is not None:
                try:
                    # Fire-and-forget release to reduce latency.
...
def init_search_execution_pool(
    ...
    if mode == PoolMode.ThreadMode:
        # NOTE:
        # - Tools are often initialized in places that only need schemas (e.g. prompt construction).
        #   Creating a non-detached actor at init-time can lead to it being garbage-collected and
        #   killed immediately, which has been observed to trigger Ray worker aborts in some setups.
        # - Make this a detached singleton so it outlives transient references.
        return ray.remote(SearchExecutionWorker).options(
...
class SearchTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        # Lazily initialize Ray execution pool to avoid creating/killing Ray actors during
        # schema-only tool initialization.
        self._execution_pool: Optional[ray.actor.ActorHandle] = None
        self._execution_pool_init_lock = threading.Lock()
        ...
        self.initial_k = config.get("initial_k", 50)
        self.max_chars = config.get("max_chars", 10000)
        self.use_reranker = config.get("use_reranker", None)
    ...
    def _get_or_create_execution_pool(self) -> ray.actor.ActorHandle:
        if self._execution_pool is not None:
            return self._execution_pool

        with self._execution_pool_init_lock:
            if self._execution_pool is None:
                self._execution_pool = init_search_execution_pool(
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        # Allow override from tool parameters (if LLM generates it) or use config default
        use_reranker = parameters.get("use_reranker", self.use_reranker)

        try:
            execution_pool = self._get_or_create_execution_pool()
            ...
            try:
                ...
                if isinstance(data, dict) and "result" in data:
                    new_result_text = json.dumps(data["result"], indent=2, ensure_ascii=False)
                    if "truncation_warning" in data and data["truncation_warning"]:
                        new_result_text += f"\n{data['truncation_warning']}"
                    result_text = new_result_text
            except json.JSONDecodeError:
                pass
            except Exception as e:
                logger.warning(f"[SearchTool] Failed to parse result: {e}")
