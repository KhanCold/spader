...
def validate_config(
    ...
    # Length sanity checks (token-based)
    # These are *warnings* (not hard errors) because some users may rely on engine-side truncation,
    # but in practice mismatches here are the most common cause of "short inputs get truncated".
    try:
        rollout_cfg = config.actor_rollout_ref.rollout
        data_cfg = config.data
        max_model_len = rollout_cfg.get("max_model_len", None)
        max_prompt_length = data_cfg.get("max_prompt_length", None)
        max_response_length = data_cfg.get("max_response_length", None)

        if max_model_len is not None and max_prompt_length is not None and max_response_length is not None:
            single_turn_needed = int(max_prompt_length) + int(max_response_length)
            if int(max_model_len) < single_turn_needed:
                print(
                    "WARNING: actor_rollout_ref.rollout.max_model_len is smaller than "
                    "data.max_prompt_length + data.max_response_length. "
                    "This will force engine-side truncation/early stopping. "
                    f"Got max_model_len={max_model_len}, max_prompt_length={max_prompt_length}, "
                    f"max_response_length={max_response_length}."
                )

        # Multi-turn often needs more slack because conversation history + tool outputs accumulate.
        mt_cfg = rollout_cfg.get("multi_turn", None)
        if mt_cfg and mt_cfg.get("enable", False) and max_model_len is not None:
            max_assistant_turns = mt_cfg.get("max_assistant_turns", None)
            if max_assistant_turns and int(max_assistant_turns) > 1:
                print(
                    "NOTICE: multi_turn is enabled. If you observe truncation, consider increasing "
                    "actor_rollout_ref.rollout.max_model_len and/or reducing max_assistant_turns, "
                    "and ensure data.max_prompt_length leaves headroom for generation."
                )
    except Exception:
        # Do not fail validation on best-effort checks.
        pass
