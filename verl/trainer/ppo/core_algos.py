...
class AdvantageEstimator(str, Enum):
    ...
    MASKED_STEP_GRPO = "masked_step_grpo"
...
@register_adv_est(AdvantageEstimator.MASKED_STEP_GRPO)
def compute_masked_step_grpo_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    **kwargs,
    ...
    r"""Masked, *step-aware* GRPO-style advantage for multi-turn tool agents.

    Fixes the common failure mode of token-wise dense GRPO for tool agents:
    token positions are not aligned across trajectories (tool spans have variable lengths),
    so computing baselines per-token mixes different logical steps.

    We instead:
    1) Extract a sequence of *steps* per trajectory:
       - each tool-processing round contributes one action step (the token right before a tool span)
       - we additionally include a final "answer" step (last assistant token)
    2) Read step rewards r_t from those action tokens.
    3) Compute step return-to-go G_t with termination mask M_t (padded steps have M=0).
    4) Compute group baseline/std per step t across the full group size (includes zeros for stopped agents).
    5) Map A_t back onto the corresponding action tokens; all other tokens get advantage 0.

    This matches the paper formulas:
      G_t^{(i)} = \sum_{k=t}^{T_max} \gamma^{k-t} r_k^{(i)} M_k^{(i)}
      A_t^{(i)} = (G_t^{(i)} - \mu_t) / sqrt(Var_t + eps)
    """
    assert config is not None, "MASKED_STEP_GRPO requires algorithm config (for gamma)"
    gamma = float(getattr(config, "gamma", 1.0))

    # response mask: (bs, response_length), 1 for assistant tokens, 0 for tool/padding tokens
    # validity_mask: (bs, response_length), 1 for valid tokens, 0 for padding

    if validity_mask is None:
        # Best-effort fallback: treat all positions as valid. This may confuse padding with tool spans
        # when response_mask==0 is used both for tool and padding.
        validity_mask = torch.ones_like(response_mask, dtype=torch.long)
    validity_mask = validity_mask.to(device=response_mask.device)

    def _extract_step_positions_and_spans(
        rm_row: torch.Tensor, valid_len: int
    ) -> tuple[list[int], list[tuple[int, int]]]:
        """Extract step anchor positions and the assistant-token spans for each step.

        We treat each tool-processing round as one logical step:
          assistant span (mask==1) -> tool span (mask==0)

        For each tool round, we:
          - anchor the step reward at the *last* token of the preceding assistant span
          - broadcast the step advantage/return across the whole preceding assistant span

        The final step is the last assistant span (after the final tool span, if any).

        Returns:
            step_positions: list[int]
                Anchor token index for reading step reward (typically the last token in the span).
            step_spans: list[(start, end)]
                Assistant-token spans [start, end) to broadcast A_step/Return.
        """
        if valid_len <= 0:
            return [], []

        rm = rm_row[:valid_len].to(dtype=torch.int64)

        step_positions: list[int] = []
        step_spans: list[tuple[int, int]] = []

        # Scan alternating assistant/tool regions.
        t = 0
        current_assistant_start: int | None = None
        while t < valid_len:
            is_assistant = int(rm[t].item()) == 1
            if is_assistant:
                if current_assistant_start is None:
                    current_assistant_start = t
                t += 1
                continue

            # rm[t] == 0 (tool/padding/tool-response tokens)
            if current_assistant_start is not None:
                assistant_end = t
                if assistant_end > current_assistant_start:
                    step_spans.append((current_assistant_start, assistant_end))
                    step_positions.append(assistant_end - 1)
                current_assistant_start = None

            # Skip over the tool span.
            while t < valid_len and int(rm[t].item()) == 0:
                t += 1

        # Trailing assistant span is the final-answer step.
        if current_assistant_start is not None and current_assistant_start < valid_len:
            step_spans.append((current_assistant_start, valid_len))
            # Anchor at the last assistant token.
            step_positions.append(valid_len - 1)

        return step_positions, step_spans

    with torch.no_grad():
        bsz, T = token_level_rewards.shape

        # 1) Build per-sample step positions, step spans, and step rewards.
        per_sample_positions: list[list[int]] = []
        per_sample_spans: list[list[tuple[int, int]]] = []
        step_lengths: list[int] = []
        for i in range(bsz):
            valid_len = int(validity_mask[i].sum().item())
            valid_len = max(min(valid_len, T), 0)
            positions, spans = _extract_step_positions_and_spans(response_mask[i], valid_len)
            # Keep the two aligned; if they diverge for any edge case, truncate conservatively.
            k = min(len(positions), len(spans))
            positions = positions[:k]
            spans = spans[:k]
            per_sample_positions.append(positions)
            per_sample_spans.append(spans)
            step_lengths.append(k)

        max_steps = max(step_lengths) if step_lengths else 0
        if max_steps == 0:
            # No steps found; return all zeros.
            print("[WARN] core_algos: no steps found in any trajectory; returning zero advantages.")
            zeros = torch.zeros_like(token_level_rewards)
            return zeros, zeros

        # r_steps: (bs, max_steps)
        # m_steps: (bs, max_steps), 1 if step exists, 0 for padded steps
        r_steps = torch.zeros(bsz, max_steps, device=token_level_rewards.device, dtype=token_level_rewards.dtype)
        m_steps = torch.zeros(bsz, max_steps, device=token_level_rewards.device, dtype=token_level_rewards.dtype)
        for i in range(bsz):
            positions = per_sample_positions[i]
            for t_step, pos in enumerate(positions):
                if 0 <= pos < T and t_step < max_steps:
                    r_steps[i, t_step] = token_level_rewards[i, pos]
                    m_steps[i, t_step] = 1.0

        # 2) Step return-to-go with termination mask M.
        G_steps = torch.zeros_like(r_steps)
        running = torch.zeros(bsz, device=r_steps.device, dtype=r_steps.dtype)
        for t in reversed(range(max_steps)):
            running = r_steps[:, t] + gamma * running
            G_steps[:, t] = running
            running = running * m_steps[:, t]

        # 3) Group-wise per-step normalization: baseline includes zeros for stopped trajectories.
        prompt_groups = defaultdict(list)
        for i in range(bsz):
            prompt_groups[index[i]].append(i)

        A_steps = torch.zeros_like(G_steps)
        for _, traj_indices in prompt_groups.items():
            traj_idx = torch.tensor(traj_indices, device=G_steps.device)
            Rg = G_steps[traj_idx]  # [G, S]
            mu = Rg.mean(dim=0)
            var = ((Rg - mu) ** 2).mean(dim=0)
            std = torch.sqrt(var + epsilon)
            A_steps[traj_idx] = (Rg - mu) / std

        # Mask out padded steps (post-termination).
        A_steps = A_steps * m_steps

        # 4) Map back to token space.
        # Broadcast each step's advantage/return across the corresponding assistant span.
        # This ensures the whole <tool_call>...</tool_call> sequence receives gradient signal,
        # while tool-response spans (mask==0) remain unaffected.
        advantages = torch.zeros_like(token_level_rewards)
        returns = torch.zeros_like(token_level_rewards)
        for i in range(bsz):
            spans = per_sample_spans[i]
            for t_step, (s, e) in enumerate(spans):
                if t_step >= max_steps:
                    break
                if e <= s:
                    continue
                # Clamp to tensor length in case of any mismatch.
                s2 = max(min(int(s), T), 0)
                e2 = max(min(int(e), T), 0)
                if e2 <= s2:
                    continue
                advantages[i, s2:e2] = A_steps[i, t_step]
                returns[i, s2:e2] = G_steps[i, t_step]
