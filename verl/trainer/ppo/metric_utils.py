...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Build histogram data for distribution logging
    histograms = {
        "hist/advantages": valid_adv.detach().cpu().numpy(),
        "hist/rewards": non_aborted_sequence_reward.detach().cpu().numpy(),
        "hist/scores": non_aborted_sequence_score.detach().cpu().numpy(),
        "hist/returns": valid_returns.detach().cpu().numpy(),
        "hist/response_length": response_length.detach().cpu().numpy(),
    }
    if use_critic:
        histograms["hist/values"] = valid_values.detach().cpu().numpy()
