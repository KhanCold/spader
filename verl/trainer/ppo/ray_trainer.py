...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # For step-aware estimators, we also provide a validity mask over the response tokens
        # to distinguish real tokens from padding (response_mask==0 is used for tool spans too).
        if adv_estimator == AdvantageEstimator.MASKED_STEP_GRPO and "attention_mask" in data.batch:
            response_length = data.batch["responses"].size(1)
            adv_kwargs["validity_mask"] = data.batch["attention_mask"][:, -response_length:]
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # One-time guardrail: if rewards are already present as `rm_scores`, the trainer will
        # extract them and bypass `reward_fn` (and thus any step-wise reward_extra_info).
        self._warned_extract_rm_scores = False
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, reward_extra_infos_dict, dump_path):
        ...
        def _json_default(obj: Any):
            # Convert common non-JSON scalar/container types into Python natives.
        ...
        n = len(inputs)
        original_n = n
        dump_limit = self.config.trainer.get("dump_val_jsonl_num", None)
        if dump_limit is not None:
            try:
                dump_limit = int(dump_limit)
            except (TypeError, ValueError):
                dump_limit = None
        if dump_limit is not None and dump_limit >= 0:
            n = min(n, dump_limit)
        ...
        for k, v in reward_extra_infos_dict.items():
            if len(v) == n or len(v) == original_n:
                base_data[k] = v[:n]
    ...
    def _compute_or_extract_reward(
        ...
        if "rm_scores" in batch.batch.keys():
            # Guardrail warning: extraction means reward_fn is bypassed.
            # This commonly happens when reward_loop/agent_loop precomputes rewards.
            if not self._warned_extract_rm_scores:
                try:
                    reward_fn_name = getattr(getattr(self.reward_fn, "__class__", None), "__name__", None)
                    val_reward_fn_name = getattr(getattr(self.val_reward_fn, "__class__", None), "__name__", None)
                except Exception:
                    reward_fn_name, val_reward_fn_name = None, None

                step = getattr(self, "global_steps", None)
                warnings.warn(
                    "Detected precomputed `rm_scores` in the batch; trainer will extract rewards and bypass "
                    "`reward_fn`/`val_reward_fn`. If you expect step-wise extras (e.g. step_rewards, "
                    "step_new_entities, group_entity_freq) to be computed and dumped, ensure your reward is "
                    "computed by the reward_manager (disable reward_loop / avoid producing rm_scores in rollout). "
                    f"(global_steps={step}, reward_fn={reward_fn_name}, val_reward_fn={val_reward_fn_name}, "
                    f"use_reward_loop={getattr(self.config.reward_model, 'use_reward_loop', None)})",
                    stacklevel=2,
                )
                self._warned_extract_rm_scores = True
    ...
    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        ...
        metric_dict = {}
        core_vars = {"f1_score", "recall", "precision"}
    ...
    def _log_histograms(self, logger, step, hist_data: dict[str, np.ndarray]):
        """Log wandb.Histogram objects. Skipped silently if wandb is unavailable.

        Args:
            logger: Tracking logger instance.
            step: Current global step.
            hist_data: Mapping from metric key to 1-D numpy array.
        """
        try:
            import wandb
        except ImportError:
        ...
        for key, arr in hist_data.items():
            try:
                hist_metrics[key] = wandb.Histogram(np.asarray(arr, dtype=np.float64).ravel())
            except Exception:
                continue
        if hist_metrics:
            logger.log(data=hist_metrics, step=step, backend=["wandb"])
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if reward_extra_infos_dict:
                            ...
                            # Train-time logging for extra reward metrics (e.g. qampari f1/precision/recall).
                            # We aggregate over the current batch (optionally per data_source).
                            try:
                            ...
                            for metric_name, metric_vals in reward_extra_infos_dict.items():
                                # Skip non-numeric / high-cardinality fields.
                                if metric_name in {"pred"}:
                                    continue
                                try:
                                    arr = np.asarray(metric_vals)
                                    if arr.dtype.kind not in "iufb":
                                        arr = arr.astype(np.float32)
                                except Exception:
                                    continue

                                # If we can, log per-datasource means; otherwise log a global mean.
                                if data_sources is not None and len(arr) == len(data_sources):
                                    try:
                                        for ds in np.unique(data_sources):
                                            mask = data_sources == ds
                                            if not np.any(mask):
                                                continue
                                            metrics[f"train-aux/{ds}/{metric_name}/mean"] = float(np.mean(arr[mask]))
                                    except Exception:
                                        metrics[f"train-aux/{metric_name}/mean"] = float(np.mean(arr))
                                else:
                                    metrics[f"train-aux/{metric_name}/mean"] = float(np.mean(arr))
                ...
                # collect metrics
                data_metrics, hist_data = compute_data_metrics(batch=batch, use_critic=self.use_critic)
                metrics.update(data_metrics)
                self._log_histograms(logger, self.global_steps, hist_data)
